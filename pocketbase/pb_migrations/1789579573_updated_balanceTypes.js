...
migrate((app) => {
  ...
  unmarshal({
    "listRule": "@request.auth.id != '' && (owner = @request.auth.id || accounts_via_balanceType.accountShares_via_account.recipient ?= @request.auth.id || assets_via_balanceType.assetShares_via_asset.recipient ?= @request.auth.id)",
    "viewRule": "@request.auth.id != '' && (owner = @request.auth.id || accounts_via_balanceType.accountShares_via_account.recipient ?= @request.auth.id || assets_via_balanceType.assetShares_via_asset.recipient ?= @request.auth.id)"
  }, collection)
