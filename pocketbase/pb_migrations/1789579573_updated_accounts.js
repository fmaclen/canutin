...
migrate((app) => {
  ...
  unmarshal({
    "listRule": "@request.auth.id != '' && (owner = @request.auth.id || accountShares_via_account.recipient ?= @request.auth.id)",
    "viewRule": "@request.auth.id != '' && (owner = @request.auth.id || accountShares_via_account.recipient ?= @request.auth.id)"
  }, collection)
