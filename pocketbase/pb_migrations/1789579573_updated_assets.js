...
migrate((app) => {
  ...
  unmarshal({
    "listRule": "@request.auth.id != '' && (owner = @request.auth.id || assetShares_via_asset.recipient ?= @request.auth.id)",
    "viewRule": "@request.auth.id != '' && (owner = @request.auth.id || assetShares_via_asset.recipient ?= @request.auth.id)"
  }, collection)
