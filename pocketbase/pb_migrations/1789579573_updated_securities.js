...
migrate((app) => {
  ...
  unmarshal({
    "listRule": "@request.auth.id != '' && (owner = @request.auth.id || securityBalances_via_security.account.accountShares_via_account.recipient ?= @request.auth.id || securityTransactions_via_security.account.accountShares_via_account.recipient ?= @request.auth.id)",
    "viewRule": "@request.auth.id != '' && (owner = @request.auth.id || securityBalances_via_security.account.accountShares_via_account.recipient ?= @request.auth.id || securityTransactions_via_security.account.accountShares_via_account.recipient ?= @request.auth.id)"
  }, collection)
