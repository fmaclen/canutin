...
func seedDemoQAData(app core.App, userID string, reference time.Time) error {
	for _, code := range []string{"USD", demoQAMissingAccountCurrency, demoQAMissingAssetCurrency} {
		if _, _, err := ensureCurrencyRecord(app, userID, code, "", false); err != nil {
			return err
		}
	}
	...
	accountType := core.NewRecord(balanceTypes)
	accountType.Set("name", "Checking")
	accountType.Set("owner", userID)
	if err := app.Save(accountType); err != nil {
		return err
	}
	assetType := core.NewRecord(balanceTypes)
	assetType.Set("name", "Other")
	assetType.Set("owner", userID)
	if err := app.Save(assetType); err != nil {
		return err
	}
	...
	accountBalances, err := app.FindCollectionByNameOrId("accountBalances")
	if err != nil {
		return err
	}
	for _, fixture := range []struct {
		...
		{"QA USD account", "USD", 200},
		{"QA QAC account", demoQAMissingAccountCurrency, 1495000},
	} {
		account := core.NewRecord(accounts)
		account.Set("name", fixture.name)
		account.Set("balanceGroup", "CASH")
		account.Set("balanceType", accountType.Id)
		account.Set("owner", userID)
		account.Set("currency", fixture.currency)
		if err := app.Save(account); err != nil {
			return err
		}
		balance := core.NewRecord(accountBalances)
		balance.Set("account", account.Id)
		balance.Set("owner", userID)
		balance.Set("asOf", demoISODate(reference))
		balance.Set("value", fixture.value)
	...
	assetBalances, err := app.FindCollectionByNameOrId("assetBalances")
	if err != nil {
		return err
	}
	for _, fixture := range []struct {
		...
		{"QA USD asset", "USD", 500},
		{"QA QAS asset", demoQAMissingAssetCurrency, 1000},
	} {
		asset := core.NewRecord(assets)
		asset.Set("name", fixture.name)
		asset.Set("balanceGroup", "OTHER")
		asset.Set("balanceType", assetType.Id)
		asset.Set("owner", userID)
		asset.Set("currency", fixture.currency)
		if err := app.Save(asset); err != nil {
			return err
		}
		balance := core.NewRecord(assetBalances)
		balance.Set("asset", asset.Id)
		balance.Set("owner", userID)
		balance.Set("asOf", demoISODate(reference))
		balance.Set("marketValue", fixture.value)
		if err := app.Save(balance); err != nil {
			return err
		}
	}
