<script lang="ts">
	...
	const transactionsById = new SvelteMap<string, TransactionsResponse>();
	...
	async function loadInitialTransactions() {
		...
		const txs = await pbClient.collection('transactions').getFullList<TransactionsResponse>({
	...
	$effect(() => {
		if (accountsContext.accounts) rebuildQueue.enqueue('rebuild');
	});
