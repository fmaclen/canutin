/*
  ...
  Env (defaults in scripts/server.ts):
    PB_HOST=127.0.0.1
    PB_PORT=42070
    PB_SUPERUSER_EMAIL=superadmin@example.com
    PB_SUPERUSER_PASSWORD=123qweasdzxc
*/
...
function baseUrl() {
  const host = getEnv('PB_HOST', '127.0.0.1')
  const port = getEnv('PB_PORT', '42070')
  return `http://${host}:${port}`
}

function mapBalanceGroup(n: number | null | undefined): 'CASH' | 'DEBT' | 'INVESTMENT' | 'OTHER' | undefined {
...
async function main() {
  ...
  const email = getEnv('PB_SUPERUSER_EMAIL', 'superadmin@example.com')
  const password = getEnv('PB_SUPERUSER_PASSWORD', '123qweasdzxc')

  log(`Authenticating as superuser at ${baseUrl()} ...`)
  try {
  ...
  async function upsertBalanceType(name: string): Promise<string> {
    ...
    try {
      const existing = await pb.collection('balanceTypes').getFirstListItem(`name = ${JSON.stringify(key)}`)
  ...
  async function upsertTxLabel(name: string): Promise<string> {
    ...
    try {
      const existing = await pb.collection('transactionLabels').getFirstListItem(`name = ${JSON.stringify(key)}`)
  ...
  const txGroups = db.query('SELECT id, name FROM TransactionCategoryGroup ORDER BY id').all() as Ref[]
  const txCategories = db
    .query('SELECT id, name, transactionCategoryId FROM TransactionCategory ORDER BY id')
    .all() as Array<
      Ref & { transactionCategoryId: number }
    >

  log(`Found: ${accountTypes.length} AccountType, ${assetTypes.length} AssetType, ${txGroups.length} Tx Groups, ${txCategories.length} Tx Categories`)
  ...
  const txCatGroupIdByCatId = new Map<number, number>(txCategories.map((r) => [r.id, r.transactionCategoryId]))
  ...
  type AccBalRow = { id: number; value: number; accountId: number; createdAt?: string; updatedAt?: string }
  const accountBalances = db
    .query('SELECT id,value,accountId,createdAt,updatedAt FROM AccountBalanceStatement ORDER BY createdAt, id')
    .all() as AccBalRow[]

  type AstBalRow = { id: number; value: number; quantity?: number | null; cost?: number | null; assetId: number; createdAt?: string; updatedAt?: string }
  ...
  log(`Rows: accounts=${accounts.length}, assets=${assets.length}, accountBalances=${accountBalances.length}, assetBalances=${assetBalances.length}, transactions=${transactions.length}`)
  ...
  for (const s of assetBalances) {
    ...
    const bal = await pb.collection('assetBalances').create({ value: s.value, quantity: s.quantity ?? undefined, cost: s.cost ?? undefined })
